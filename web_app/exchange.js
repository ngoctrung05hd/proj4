...
const token_name = "Fauzhandsome"; // TODO: replace with name of your token
const token_symbol = "FZH"; // TODO: replace with symbol for your token
...
const token_address = "0xD1182C929819279d66d9B11d91d7e2C59e8C0725";
const token_abi = [
    {
      "inputs": [],
...
const exchange_address = "0x8762FFdF426BA7be374F4D962Bd47AD108a8DfA7";
const exchange_abi = [
    {
      "inputs": [],
      "stateMutability": "nonpayable",
...
async function getExchangeRate(maxSlippagePct) {
  const poolRate = (await getPoolState())["eth_token_rate"];
  const exchange_rate_multiplier = 10 ** 5;
  const min_exchange_rate = Math.ceil((exchange_rate_multiplier * poolRate * (100 - maxSlippagePct)) / 100);
  const max_exchange_rate = Math.floor((exchange_rate_multiplier * poolRate * (100 + maxSlippagePct)) / 100);
...
async function addLiquidity(amountEth, maxSlippagePct) {
  /** TODO: ADD YOUR CODE HERE **/
	const rate = await getExchangeRate(maxSlippagePct);
	const amountToken = Math.floor(amountEth * (await getPoolState())["token_eth_rate"]); 
	await token_contract.connect(provider.getSigner(defaultAccount)).approve(exchange_address, amountToken);
    await exchange_contract.connect(provider.getSigner(defaultAccount)).addLiquidity(rate.min_exchange_rate, rate.max_exchange_rate, {value: ethers.utils.parseEther(amountEth.toString())});
}
...
async function removeLiquidity(amountEth, maxSlippagePct) {
  /** TODO: ADD YOUR CODE HERE **/
	const rate = await getExchangeRate(maxSlippagePct);
    await exchange_contract.connect(provider.getSigner(defaultAccount)).removeLiquidity(ethers.utils.parseEther(amountEth.toString()), rate.min_exchange_rate, rate.max_exchange_rate);
}

async function removeAllLiquidity(maxSlippagePct) {
  /** TODO: ADD YOUR CODE HERE **/
	const rate = await getExchangeRate(maxSlippagePct);
    await exchange_contract.connect(provider.getSigner(defaultAccount)).removeAllLiquidity(rate.min_exchange_rate, rate.max_exchange_rate);
}
...
async function swapTokensForETH(amountToken, maxSlippagePct) {
  /** TODO: ADD YOUR CODE HERE **/
	const rate = await getExchangeRate(maxSlippagePct);
	await token_contract.connect(provider.getSigner(defaultAccount)).approve(exchange_address, amountToken);
    await exchange_contract.connect(provider.getSigner(defaultAccount)).swapTokensForETH(amountToken, rate.max_exchange_rate);
}

async function swapETHForTokens(amountEth, maxSlippagePct) {
  /** TODO: ADD YOUR CODE HERE **/
	const rate = await getExchangeRate(maxSlippagePct);
    await exchange_contract.connect(provider.getSigner(defaultAccount)).swapETHForTokens(rate.max_exchange_rate, {value: ethers.utils.parseEther(amountEth.toString())});
}
